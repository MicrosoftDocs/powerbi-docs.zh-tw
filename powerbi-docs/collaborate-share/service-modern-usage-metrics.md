...
- 在某些情況下，您可能會注意到效能資料遺失。 如果使用者在完成載入之前開啟報表並與報表互動，或在報表載入期間發生錯誤，就會發生這種情況。
- 效能資料會依賴將資料傳送至 Power BI 的用戶端/裝置。 根據組織所設定的網路延遲、廣告封鎖程式、防火牆和網路規則，效能資料也可能永遠無法傳送至 Power BI。 因此，效能資料只能以範例形式呈現，且無法包含或顯示所有使用者。 

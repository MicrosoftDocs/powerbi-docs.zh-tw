...
* **複合模型**：允許報表有兩個或更多個來自不同來源群組的資料連線，例如一或多個 DirectQuery 連線及一個匯入連線、兩個或更多個 DirectQuery 連線，或上述項目的任意組合。 本文會詳細描述複合模型。
...
如需儲存模式的詳細資訊，請參閱[管理 Power BI Desktop 中的儲存模式](desktop-storage-mode.md)。  
...
此外，如果作者將「模型 A」中的「資料表 1」新增至複合模型 (以下統稱「模型 C」)，則當使用者檢視建置於「模型 C」中的報表時，即可查詢在「模型 A」中不受 RLS 保護的 **任何資料表**。

...
本文描述將範本應用程式安裝設定自動化所需的基本流程、必要條件，以及主要步驟與 API。 不過，如果想要立刻深入了解並開始使用，則可跳至[教學課程](template-apps-auto-install-tutorial.md)，在該教學課程中，您可使用我們已備妥的簡單範例應用程式 (使用 Azure 函數)，以將範本應用程式安裝的設定自動化。

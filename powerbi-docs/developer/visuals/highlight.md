...
建立 `div` 和 `p` 元素，以顯示視覺效果 DOM 中的資料檢視值並將其視覺化：
...
套用元素所需的樣式，以使用 `flex box` 並定義 div 元素的色彩：
...
建立預設的視覺效果專案，並套用 `capabilities.json` 的範例。
...
使用醒目提示的主要步驟是處理額外的值陣列。

如果您檢查終端機節點的物件，即可以看到值陣列有兩個屬性 (value 和 highlight)：
...
* [了解矩陣資料檢視對應](dataview-mappings.md#matrix-data-mapping)

* [了解視覺效果的功能](capabilities.md)
